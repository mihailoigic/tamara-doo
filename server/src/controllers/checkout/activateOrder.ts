...
export const activateOrder = async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    ...
    try {
        ...
        if(korpa){
            ...
            res.customSuccess(200, `Status updated to active for [${id}]`, { code: 0, brojKupovine: id});
        } else{
