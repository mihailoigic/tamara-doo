...
export class DiscountKategorija {
    ...
    @ManyToOne(() => Discount, (discount) => discount.kategorije, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    @JoinColumn([{ name: 'fordiscount', referencedColumnName: 'id' }])
    forKategorijaDiscount: Proizvod;

    @ManyToOne(() => KategorijeSifrarnik, (kategorija) => kategorija.kategorijaDiscount, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
