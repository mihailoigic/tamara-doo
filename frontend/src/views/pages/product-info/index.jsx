        ...
        <>
            <div className="product-info-container" onClick={() => history.push('/product')} onMouseOver={()=>setShowSizes(true)} onMouseLeave={()=>setShowSizes(false)}>
                ...
                <div className="product-details pt-1 ps-2">
                    <p className="ff-releway color-first mb-0">{firstLetter(product.name)}</p>
                    <p className="mb-0 color-secondary">{(product.brand).toUpperCase()}</p>
                    <p className="mb-0 color-secondary">{moda}</p>
                    <Boje nameOfClass='color-item-product' product={product} imgClass='rounded-1'/>
                </div>
            </div>
        </>
    );
